...
if (!$uniface_file){
	...
	### Signatures ###		
	} elseif ($working_dir.EndsWith("signatures")){
		Write-Host "Signature : 		: $uniface_object"
		
		# export the USSPEC and wait for the job to finish
		Write-Host "Export USSPEC   - Signature Specifications"
		& $executable /adm=$administration /cpy /whr="USPECNAM=$uniface_object" IDF:USSPEC.DICT XML:$uniface_file | Out-Null

		# export the USOPER and wait for the job to finish
		Write-Host "Export USOPER - Signature Operations"
		& $executable /adm=$administration /cpy /apf /whr="USPECNAM=$uniface_object" IDF:USOPER.DICT XML:$uniface_file | Out-Null
		
		# export the USPARM and wait for the job to finish
		Write-Host "Export USPARM - Signature Parameters"
		& $executable /adm=$administration /cpy /apf /whr="USPECNAM=$uniface_object" IDF:USPARM.DICT XML:$uniface_file | Out-Null

		# export the USIMPL and wait for the job to finish
		Write-Host "Export USIMPL - Signature Implementation"
		& $executable /adm=$administration /cpy /apf /whr="USPECNAM=$uniface_object" IDF:USIMPL.DICT XML:$uniface_file | Out-Null

		# export the USIOPER and wait for the job to finish
		Write-Host "Export USIOPER - Signature Implementation Operations"
		& $executable /adm=$administration /cpy /apf /whr="USPECNAM=$uniface_object" IDF:USIOPER.DICT XML:$uniface_file | Out-Null

		# export the USIPARM and wait for the job to finish
		Write-Host "Export USIPARM - Signature Implementation Parameters"
		& $executable /adm=$administration /cpy /apf /whr="USPECNAM=$uniface_object" IDF:USIPARM.DICT XML:$uniface_file | Out-Null

	### Library Objects ###		
	} elseif ($working_dir.EndsWith("include_procs")){
		# export the library and wait for the job to finish 
		Write-Host "Export ULIBR    - Library"
		& $executable /adm=$administration /cpy /whr="ULIBRARY=$appl_model" IDF:ULIBR.DICT XML:$uniface_file | Out-Null
		...
		# export the <tablename> 
		Write-Host "Export USOURCE	- Sources Include Procs"
		& $executable /adm=$administration /cpy /apf /whr="UCVAR=$appl_model;UCLABEL=$uniface_object" IDF:UGLYPH.DICT XML:$uniface_file | Out-Null
